...
/** True if this function is audio we should try (VIA AC97, ICH AC97, class 04). */
static int pci_is_audio(uint16_t vendor, uint16_t device, uint8_t cc, uint8_t sc)
{
	if (cc == 0x04) {
		return 1;
	}
	...
	for (uint16_t bus = 0; bus < 8; bus++) {
		for (uint8_t slot = 0; slot < 32; slot++) {
			...
			for (uint8_t func = 0; func < funcs; func++) {
				...
				if (!pci_is_audio(vendor, device, cc, sc) || device_count >= PCI_MAX_DEVICES) {
					continue;
				}
				...
				d->subclass = sc;
				for (int i = 0; i < 6; i++) {
					d->bar[i] = pci_read32((uint8_t)bus, slot, func, (uint8_t)(0x10 + i * 4));
				}
			}
		}
	}
